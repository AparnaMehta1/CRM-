...
export default function Admin() {
  ...
  const onTicketUpdate = (e) => {
    if (e.target.name === "title") {
      ...
      ticketUpdateCurr.reporter = e.target.value
    } else if (e.target.name === "status") {                 
      ticketUpdateCurr.status = e.target.value
    } else if (e.target.name === "ticketPriority") {
      ticketUpdateCurr.ticketPriority = e.target.value
    }
  ...
  const updateUserDetail = () => {
    const data = {
      "userType": userList.userTypes,
      "userStatus": userList.userStatus,
      "userName": userList.name,
      "email": userList.email,
      "userId": userList.userId
    }
    updateUserData(userList.userId, data).then(function (response) {
      if (response.status === 200) {
        setMessage(response.message);
        let idx = userList.findIndex((obj => obj.userId === userList.userId));
        userList[idx] = userList
        closeUserModal();
  ...
  const changeUserDetail = (e) => {
    if (e.target.name === "status")
      userList.userStatus = e.target.value
    else if (e.target.name === "userName")
      userList.name = e.target.value
    else if (e.target.name === "type")
      userList.userTypes = e.target.value
    else if (e.target.name === "email")
      userList.email = e.target.value
    setUserList(userList)
    setUserModal(e.target.value);
  }

  return (
    <div className="admin vh-100">
      <div className="row">
        <div className="col-1">
          <Sidebar />
        </div>
        <div className="container col m-1">
          <h3 className="text-primary text-center fw-bold"> Welcome_Admin </h3>
              ...
              <div className="col my-1 p-2">
                <div className="card" style={{ width: 12 + 'rem' }}>
                  <div className="box borders_b">
                    <h5 className="card-subtitle my-1 fw-bold text-primary">
                      <BorderColorIcon />
                      Open
                    </h5>
                    <hr />
          ...
          {!showTable ?
            <div className="container">
              <div className="text-success">{message.includes("Ticket") ? message : ""}</div>
              <center><button onClick={userDetail} className="btn btn-lg fw-bolder text-dark">User Details</button></center>
              <div className="table">
                <MaterialTable
            ...
            <div className="container">
              ...
              <center><button onClick={adminDetail} className="btn fw-bolder btn-lg text-dark">Admin Details</button></center>
              <div className="table">
                <MaterialTable
                  onRowClick={(event, rowData) => updateUserDetail(rowData.userId)}
                  columns={[
                    {
                      title: 'User ID',
                      field: 'userId'
          ...
          {userModal ? (
            ...
            <Modal.Body> 
              <form onSubmit={updateUserDetail}>
                <div className="p-1">
                  <h5 className="card-subtitle mb-2 text-primary lead">User ID: {userList.userId}</h5>
                  <hr />
                  <div className="input-group mb-3">
                    <label className="label input-group-text label-md ">Name</label>
                    <input type="text" className="form-control" name="userName" value={userList.name} onChange={changeUserDetail} />
                  </div>
                  <div className="input-group mb-3">
                    <label className="label input-group-text label-md ">Email</label>
                    <input type="email" className="form-control" name="email" value={userList.email} onChange={changeUserDetail} disabled/>
                  </div>
                  <div className="input-group mb-3">
                    <label className="label input-group-text label-md ">Type</label>
                    <select className="form-select" name="type" value={userList.userTypes} onChange={changeUserDetail}>
                  ...
                  <div className="input-group mb-3">
                    ...
                    <select name="status" className="form-select"
                      value={userList.userStatus} onChange={changeUserDetail}>
                ...
                <form onSubmit={updateTicket}>
                  <div className="p-1">
                    ...
                    <div className="input-group">
                      ...
                      <input type="text" className="form-control" name="reporter" value={ticketUpdateCurr.reporter} onChange={onTicketUpdate} />
                    </div>
                    <div className="input-group">
                      <label className="label input-group-text label-md"> Status </label>
                      <input type="text" className="form-control" name="ticketPriority" value={ticketUpdateCurr.ticketPriority} onChange={onTicketUpdate} />
                    </div>
                    <div className="input-group">
                      <label className="label input-group-text label-md"> Status </label>
                      <input type="text" className="form-control" name="status" value={ticketUpdateCurr.status} onChange={onTicketUpdate} />
                    </div>
                    <div className="input-group">
